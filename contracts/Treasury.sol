...
contract Treasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SAte }
    ...
    address public sAte;
    uint public sAteQueue; // Delays change to sAte address
    ...
    /**
        @notice allow approved address to deposit an asset for Ate
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint Ate needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( Ate ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn Ate for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IAteERC20( Ate ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sAte ).balanceOf( msg.sender ); // Can only borrow against sAte held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with Ate
        @param _amount uint
     */
    function repayDebtWithAte( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IAteERC20( Ate ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, Ate, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( Ate ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( Ate ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns Ate valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match Ate decimals
            value_ = _amount.mul( 10 ** IERC20( Ate ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SAte ) { // 9
            sAteQueue = block.number.add( blocksNeededForQueue );
        } else return false;
